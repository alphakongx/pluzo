  ...
  if (currentMessage.type === "close" || currentMessage.stream_info.count === 0) {
    return (
      <View style={styles.endContainer}>
        ...
        <View
          style={[styles.liveContentContainer, styles.endContentPadding]}
          onLayout={e => setHeight(e.nativeEvent.layout.height)}
        >
          <Text numberOfLines={1} style={styles.liveNameText}>
            {currentMessage.stream_info.name === ""
              ? "No Name"
              : currentMessage.stream_info.name}
          </Text>
          <Text style={styles.liveCreatorText}>{currentMessage.user.first_name}</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <Touchable
      onPress={() => {
        ...
        if (
          (names.length > 1 &&
            parseInt(names[1], 10) === parseInt(currentUser._id, 10)) ||
        ...
        if (
          currentMessage.stream_info.ban_list.filter(
    ...
    >
      ...
      >
        ...
        <BoxShadow setting={shadowOption} />
        <View
          style={styles.liveContentContainer}
          onLayout={e => setHeight(e.nativeEvent.layout.height)}
        >
